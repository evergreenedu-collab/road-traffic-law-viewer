...
function renderArticleBody(art){
  ...
  let out='';
  if(art.조문내용) out += `<div class="fp-para">${esc(art.조문내용)}</div>`;
  const 항=(art.항)||[];
...
function renderSubLawHistory(lawType){
  ...
  html+=`</div>`;
  // 캐스케이드 안내 박스 (사용자 결정: 캐스케이드는 법률 진입으로 안내)
  const relLaws = (lawType==='시행령' ? decreeToLaw[jo] : ruleToLaw[jo]) || [];
  html+=`<div style="margin-bottom:14px;padding:10px 14px;background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;font-size:12px;color:#1e40af;line-height:1.7">`;
  html+=`🛈 이 화면은 <b>${lawType} 제${jo}조</b> 자체의 전후비교만 보여줍니다. 법률 개정과 연결된 <b>캐스케이드 연혁</b>은 위임 근거 법률 조문에서 확인하세요.`;
  if(relLaws.length){
    html+=`<div style="margin-top:8px;display:flex;gap:6px;flex-wrap:wrap">`;
    for(const lj of relLaws){
      const lawArt=(artData.법률&&artData.법률.조문&&artData.법률.조문[lj])||{};
      const lt=lawArt.조문제목||'';
      html+=`<button onclick="switchLawType('법률');document.getElementById('sel').value='${lj}';switchTab('history');" style="padding:5px 10px;background:#fff;border:1px solid #bfdbfe;border-radius:16px;cursor:pointer;font-size:12px;color:#1e40af">법률 제${lj}조${lt?' '+esc(lt):''} 연혁 보기</button>`;
    }
    html+=`</div>`;
  }
  html+=`</div>`;
...
function popupArticle(joKey){
  ...
  body += `<button onclick="closeModal();switchLawType('법률');goArticle('${joKey}')" style="padding:9px 18px;background:var(--law);color:#fff;border:none;border-radius:6px;cursor:pointer;font-size:13px;font-weight:500">📂 이 조문 화면으로 이동</button>`;
