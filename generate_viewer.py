...
window.addEventListener('message',e=>{
  ...
  // PR-H9-fix: renderHistory 1회만 호출하도록 순서 최적화 (사용자 보고 "2초 멈춤"):
  // 이전 순서 — switchLawType → goArticle(render) → switchTab(renderHistory) → 2번 렌더
  // 새 순서  — currentTab을 history로 직접 설정 → switchLawType 또는 sel.value → renderHistory 1번

  // 1) 즉시 화면을 history로 전환 (탭 표시만, render는 호출 X)
  currentTab = 'history';
  ...
  // 2) 사용자에게 즉시 시각 피드백 (renderHistory의 스피너가 viewport 안에 보이도록 스크롤)
  if (historyEl) historyEl.innerHTML = '<div class="main"><div style="text-align:center;padding:60px 20px;color:var(--law)"><div style="font-size:36px;margin-bottom:14px">📜</div><div style="font-weight:600;font-size:15px">제'+targetJo+'조 연혁 이동 중...</div></div></div>';
  window.scrollTo({top: 0, behavior: 'smooth'});

  // 3) 법률 토글 (필요 시) — switchLawType이 buildOpts·popGrouped + renderHistory 호출
  if (typeof switchLawType === 'function' && currentLawType !== '법률') {
    switchLawType('법률');
  }

  // 4) sel.value 설정 후 renderHistory 1회 — 이전엔 render+renderHistory 두 번이었음
  const sel = document.getElementById('sel');
  if (sel) {
